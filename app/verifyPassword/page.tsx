...
export default function ResetPasswordPage() {
    ...
    const verifyCode = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setStatus("error");
                setMessage(data.message || "Kode verifikasi salah atau sudah kadaluarsa");
            }
        } catch (err) {
            setStatus("error");
    ...
    const resendCode = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/lupaPassword", {  // ✅ FIXED: Sesuai backend Anda
                method: "POST",
                headers: { "Content-Type": "application/json" },
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-500 to-orange-600 text-black p-4">
            <div className="bg-zinc-900 p-8 rounded-2xl shadow-2xl w-full max-w-md relative border border-zinc-700 text-white animate-in fade-in zoom-in duration-500">
                ...
                <div className="mb-6 p-3 bg-zinc-800/50 border border-zinc-700 rounded-xl text-center">
                    ...
                    <p className="font-medium text-white truncate">{form.email || "Tidak ada email"}</p>
                </div>
                ...
                <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                    {step === "code" ? (
                        <>
                            ...
                            <div>
                                <label className="block text-sm font-medium mb-2">Kode Verifikasi (6 digit)</label>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="w-full bg-gradient-to-r from-yellow-500 to-orange-600 text-black font-semibold py-3 px-4 rounded-xl hover:scale-105 hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
                    >
                        {loading ? (
                            <>
